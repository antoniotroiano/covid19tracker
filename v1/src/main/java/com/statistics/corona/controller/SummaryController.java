...
import com.statistics.corona.model.SummaryToday;
...
@Controller
@Slf4j
@RequestMapping("v1/covid19/daily")
...
public class SummaryController {
    ...
    public String showSummaryOfSelectedCountry(@PathVariable("country") String country, Model model) throws IOException {
        ...
        if (summaryTodayList.isEmpty() || summaryTodayList.size() <= 1) {
            model.addAttribute("noDataForDayOne", "At the moment no data for day one until today for "
                    + country + " available. Please try again later.");
